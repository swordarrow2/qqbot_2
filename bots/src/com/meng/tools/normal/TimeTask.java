...
public class TimeTask implements Runnable {
    ...
    private static TimeTask instance;

    public static TimeTask getInstance() {
        return instance;
    }

    public static TimeTask getInstance(BotWrapper botWrapper) {
        if (instance != null) {
            return instance;
        }
        instance = new TimeTask(botWrapper);
        return instance;
    }
    ...
    private long[] dnfGroup = {
            672350495L, 763223937L
    };
    ...
    public TimeTask(BotWrapper botWrapper) {
        this.botWrapper = botWrapper;
//        addTask(23, 0, () -> sendRegards(evening, true));
//        addTask(6, 0, () -> sendRegards(morning, false));
        addTask(0, 0, () -> UserInfoManager.getInstance().onNewDay());
...
//                    }
                if (c.get(Calendar.DAY_OF_WEEK) == Calendar.WEDNESDAY) {
                    for (long l : dnfGroup) {
                        botWrapper.getBotMessageHandler().sendGroupMessage(l, "都星期三了不会还有人没打完周常吧");
                        try {
                            Thread.sleep(1000);
                ...
                if (c.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY) {
                    for (long l : dnfGroup) {
                        botWrapper.getBotMessageHandler().sendGroupMessage(l, "都星期五了不会还有人没打完团吧");
                        try {
                            Thread.sleep(1000);
                        } catch (InterruptedException e) {
                            throw new RuntimeException(e);
    ...
    private boolean getTipHour(Calendar c) {
        return (c.get(Calendar.HOUR_OF_DAY) == 0 || c.get(Calendar.HOUR_OF_DAY) == 12 || c.get(Calendar.HOUR_OF_DAY) == 16 || c.get(Calendar.HOUR_OF_DAY) == 22);
    }
